...
const site_desc =
    'Wortheum is a web3 news platform where everyone gets paid for creating and curating news/articles. It leverages a robust digital points system (Worth) for digital rewards.';
...
export default function extractMeta(chain_data, rp) {
    ...
    if (rp.username && rp.slug) {
        ...
        if (content && content.id !== '0.0.0') {
            ...
            const url = 'https://wortheum.news' + d.link;
            const canonicalUrl = makeCanonicalLink(d);
            const title = d.title + ' — Wortheum';
            ...
            metas.push({
                name: 'twitter:image',
                content:
                    image || 'https://wortheum/images/wortheum.png',
            });
        } else {
            addSiteMeta(metas);
        }
        ...
        if (about == null)
            about =
                'Join thousands on wortheum who share and curate , news articles and earn rewards.';
        if (profile_image == null)
            profile_image = 'https://wortheum.news/images/wortheum.png';
        // Set profile tags
